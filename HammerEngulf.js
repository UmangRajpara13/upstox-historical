...
const filePath = path.join('Results', date, `HammerEngulf_30D_10cr_${hours}h-${minutes}m-${seconds}s.xlsx`)
                ...
                for (let j = all_days.length - 1; j > 1; j--) {
                    ...
                    if (j <= all_days.length - (X_Days + 1)) {
                        ...
                        if (low <= Lowest_of_X_Days // finding the low over X_Days 
                            ...
                            if (((t_p_1_high - t_close) / t_close) * 100 >= 1) { // target 1% achieved same day
                                worksheet.addRow({
                                    name: stock.tradingsymbol,
                                    date: `${date.getDate()} ${shortMonthNames[date.getMonth()]} ${date.getFullYear()}`,
                                    // hammerColor: close > open ? 'G' : 'R',
                                    opensAt: Math.round((((t_p_1_open - t_close) / t_close) * 100) * 100) / 100,
                                    closeToHigh: Math.round((((t_p_1_high - t_close) / t_close) * 100) * 100) / 100,
                                    // lowToLow: Math.round((((t_low - low) / low) * 100) * 100) / 100,
                                    closesAt: Math.round((((t_p_1_close - t_close) / t_close) * 100) * 100) / 100,
                                    one_percent_or_close: 0,
                                    // t_p_1_opens_at: Math.round((((t_p_1_open - t_close) / t_close) * 100) * 100) / 100,
                                ...
                                for (let k = j - 3; k > 1; k--) { // -3 coz we have to account for engulf formation
                                    const kDay = all_days[k] // Trading Day +1
                                    ...
                                    if (((kHigh -  t_close) / t_close) * 100 >= 1) {
                                        // console.log('j', j, 'k', k)
                                        worksheet.addRow({
                                            name: stock.tradingsymbol,
                                            date: `${date.getDate()} ${shortMonthNames[date.getMonth()]} ${date.getFullYear()}`,
